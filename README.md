...
3. Login to the IBM Cloud using the [Developer Tools CLI](https://www.ibm.com/cloud/cli):
...
```
and locate the `Public IP`. This IP is used to access the worklog API and UI (Flask Application). Update the `env` values in both [deploy-dataservice.yml](deploy-dataservice.yml) and [deploy-webapp.yml](deploy-webapp.yml) to the `Public IP`, replacing `<INGRESS_SUBDOMAIN>` with the `Public IP`.

4. To deploy the services to the IBM Cloud Kubernetes Service:
